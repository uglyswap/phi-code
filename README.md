...
- [Permissions & Containerization](#permissions--containerization)
- [Build from Source](#build-from-source)
- [Building standalone binaries from release source](#building-standalone-binaries-from-release-source)
- [Supply-chain hardening](#supply-chain-hardening)
- [Bundled browser engine](#bundled-browser-engine)
- [Share your OSS coding agent sessions](#share-your-oss-coding-agent-sessions)
...
- **Parallel exploration (opt-in).** `/plan --fanout` runs up to 2 read-only sub-explorers (architecture / impacted files / risks) before phase 1 and merges their findings, applying the perspective-diverse pattern to the front of the pipeline. It caps concurrency at 2 against the single key, falls back to sequential on a rate limit, and degrades to the normal single-agent EXPLORE on any failure. Off by default.
...
| `/plan --fanout` | orchestrator | Opt-in: run up to 2 concurrent read-only sub-explorers before phase 1 and merge their findings (sequential fallback on rate limit; degrades to normal EXPLORE on any failure) |
...
Phi Code does not include a built-in permission system for restricting filesystem, process, network, or credential access. By default, it runs with the permissions of the user and process that launched it.
...
|---------|----------|-------------|
| `packages/tui` | `phi-code-tui` | Terminal UI library with differential rendering |
| `packages/telemetry` | `phi-code-telemetry` | Vendor-neutral telemetry contracts, reference adapter, conformance tests, and typed schemas |
| `packages/ai` | `phi-code-ai` | Unified multi-provider LLM API (OpenAI, Anthropic, Google, etc.) |
| `packages/agent` | `phi-code-agent` | Agent runtime with tool calling and state management |
| `packages/protocol` | `phi-code-protocol` | Wire protocol shared by the session client and server |
| `packages/client` | `phi-code-client` | Programmatic client for a running session |
| `packages/server` | `phi-code-server` | Session server |
| `packages/session-backends/sqlite-node` | `phi-code-session-backend-sqlite-node` | SQLite session storage backend for Node |
| `packages/coding-agent` | `@phi-code-admin/phi-code` | Interactive coding agent CLI (binary `phi`), extensions, skills |
...
If you use Phi Code, Pi, or other coding agents for open source work, please share your sessions.
...
You can also watch [this video](https://x.com/badlogicgames/status/2041151967695634619), where Mario shows how he publishes his `pi-mono` sessions.

Upstream `pi-mono` work sessions are published here:
...
See [CONTRIBUTING.md](CONTRIBUTING.md) for contribution guidelines and [AGENTS.md](AGENTS.md) for project-specific rules (for both humans and agents). Longer term plans for upstream Pi can also be found in [RFCs](https://rfc.earendil.com/keyword/pi/).
