...
// camofox-browser is a vendored JavaScript package: its single TypeScript file
// imports real .js modules, so a .ts specifier would not resolve.
const ignoredPackages = new Set(["camofox-browser"]);
const files = [];
...
function collectTypescriptFiles(directory) {
	for (const entry of readdirSync(directory, { withFileTypes: true })) {
		...
		if (entry.isFile() && entry.name.endsWith(".ts") && !entry.name.endsWith(".d.ts")) {
			...
			if (packageIndex >= 0 && ignoredPackages.has(segments[packageIndex + 1] ?? "")) continue;
			files.push(path);
		}
	}
}
