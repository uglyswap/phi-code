...
const packages = [
	{ directory: "packages/telemetry", name: "phi-code-telemetry" },
	{ directory: "packages/ai", name: "phi-code-ai" },
	{ directory: "packages/tui", name: "phi-code-tui" },
	{ directory: "packages/agent", name: "phi-code-agent" },
	{ directory: "packages/protocol", name: "phi-code-protocol" },
...
Options:
  ...
  --skip-test          Do not run npm run test before building
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "phi-local-release-"));
	}
...
if (rootPackageJson.name !== "phi-code-monorepo") {
