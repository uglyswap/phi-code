...
```bash
npm install @phi-code-admin/mom
```
...
- run interactive coding agent session: `npx @phi-code-admin/phi-code`
- enter `/login` command
...
Update mom anytime with `npm install -g @phi-code-admin/mom`. This only updates the Node.js app on your host. Anything mom installed inside the Docker container remains unchanged.
