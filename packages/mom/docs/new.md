...
```typescript
import { type AgentSessionEvent } from "@phi-code-admin/phi-code";
...
import type { MomCustomTool, ToolAPI } from "@phi-code-admin/mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `phi-code-ai` and `@phi-code-admin/mom` are aliased to the running mom bundle.
