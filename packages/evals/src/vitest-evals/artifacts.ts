...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "phi-code-evals:session" && artifact.type !== "phi-code-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "phi-code-evals:session" ? "sessions" : "sources";
		for (const attachment of artifact.attachments) {
			...
			// Artifact references travel inside the report: keep them POSIX so a run
			// produced on Windows stays readable and comparable everywhere.
