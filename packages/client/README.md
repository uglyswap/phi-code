...
```ts
import { PiClient, type ByteTransportFactory } from "phi-code-client";
...
```ts
import { PiClient } from "phi-code-client";
import { createUnixTransportFactory } from "phi-code-client/unix";
...
The `phi-code-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `phi-code-client/unix` subpath.
