 ...
 *
 * Existing apps switch imports from "phi-code-ai" to
 * "phi-code-ai/compat" unchanged; new code uses `createModels()`
...
/**
 * @deprecated Static catalog read. Use `getBuiltinModel` from "phi-code-ai/providers/all" or `Models.getModel()`.
 *
 * Overloaded so a runtime `string` is accepted and typed as possibly missing. Callers that
 * hold a provider/model id read from config or user input (web-ui's `ProviderKeyInput`, the
 * /keys and /benchmark extensions) cannot narrow to the literal union, and the honest return
 * type is what makes their `if (!model) return false` guards typecheck instead of pretending
 * a `Model` always comes back.
 */
export const getModel = getBuiltinModel as typeof getBuiltinModel &
	((provider: string, modelId: string) => Model<Api> | undefined);

/**
 * @deprecated Static catalog read. Use `getBuiltinModels` from "phi-code-ai/providers/all" or `Models.getModels()`.
 *
 * Overloaded like `getModel`: an unknown provider read at runtime yields an empty array
 * rather than a type error at the call site.
 */
export const getModels = getBuiltinModels as typeof getBuiltinModels & ((provider: string) => Model<Api>[]);

/** @deprecated Static catalog read. Use `getBuiltinProviders` from "phi-code-ai/providers/all" or `Models.getProviders()`. */
