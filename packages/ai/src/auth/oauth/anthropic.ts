...
async function loginAnthropic(interaction: ProviderAuthInteraction): Promise<OAuthCredential> {
	...
	// phi: the anti-CSRF `state` must NOT be the PKCE verifier. Sending the verifier as
	// `state` leaks the code_verifier through the authorization URL (browser history,
	// referrers, proxy/server logs) and defeats the point of PKCE. Use an independent
	// nonce, exactly like the OpenAI Codex flow in this same directory already does.
