...
export async function processResponsesStream<TApi extends Api>(
	...
	// multi-turn replay stateless. See https://github.com/uglyswap/phi-code/issues/6409.
	...
	for await (const event of openaiStream) {
		if (event.type === "response.created") {
			...
			// phi: `\`...\` || "Unknown error"` was dead code — a template literal is always
			// truthy, so the fallback never fired and users saw "Error Code undefined: undefined"
			// whenever the provider omitted code/message. Build the message from what exists.
			const code = event.code ?? "unknown";
			const message = event.message ?? "";
			throw new Error(message ? `Error Code ${code}: ${message}` : `Error Code ${code}`);
