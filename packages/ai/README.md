...
```bash
npm install phi-code-ai
```

TypeBox exports are re-exported from `phi-code-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from 'phi-code-ai';
import { builtinModels } from 'phi-code-ai/providers/all';
...
```typescript
import { anthropicProvider } from 'phi-code-ai/providers/anthropic';
import { openaiProvider } from 'phi-code-ai/providers/openai';
import { openrouterProvider } from 'phi-code-ai/providers/openrouter';
import { amazonBedrockProvider } from 'phi-code-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from 'phi-code-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from 'phi-code-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from 'phi-code-ai';
...
```typescript
import { Type, type Tool, StringEnum } from 'phi-code-ai';
...
```typescript
import { validateToolCall, type Tool } from 'phi-code-ai';
...
```typescript
import { builtinImagesModels } from 'phi-code-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from 'phi-code-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from 'phi-code-ai';
import { openAICompletionsApi } from 'phi-code-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from 'phi-code-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from 'phi-code-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from 'phi-code-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `phi-code-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from 'phi-code-ai';
import { anthropicProvider } from 'phi-code-ai/providers/anthropic';
import { openaiProvider } from 'phi-code-ai/providers/openai';
import { googleProvider } from 'phi-code-ai/providers/google';
...
```typescript
import { createModels } from 'phi-code-ai';
import { anthropicProvider } from 'phi-code-ai/providers/anthropic';
...
- `phi-code-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `phi-code-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `phi-code-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `phi-code-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `phi-code-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from 'phi-code-ai/api/bedrock-converse-stream.lazy';
...
npx phi-code-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `phi-code-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from 'phi-code-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from 'phi-code-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `phi-code-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `phi-code-ai`.
