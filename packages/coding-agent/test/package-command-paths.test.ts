...
`,
		...
		const activeVersion = getNewerPatchVersion();
		// The npm registry manifest publishes the package name under `name`, not `packageName`.
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => Response.json({ name: activePackageName, version: activeVersion })),
		);
		...
		try {
			...
			expect(recordedCalls).toEqual([
				...
				expect.arrayContaining(["install", "-g", `${activePackageName}@${activeVersion}`]),
			]);
		} finally {
			logSpy.mockRestore();
			errorSpy.mockRestore();
