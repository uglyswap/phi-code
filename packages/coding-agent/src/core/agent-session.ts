...
import { contentText } from "phi-code-ai";
import type {
	AssistantMessage,
	AuthResult,
	ImageContent,
...
export class AgentSession {
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		// The auto-compaction race the fork guarded against with abortCompaction() is
		// now handled upstream (#7370): auto-compaction runs inside the agent run, so
		// _isAgentRunActive stays true and abort() -> waitForIdle() waits for it to
	...
	private async _prepareRetry(message: AssistantMessage): Promise<boolean> {
		...
		// re-enters the agent from here. _prepareRetry() only prepares state and returns
		// true; _handlePostAgentRun() awaits agent.continue() in the run loop.
		return true;
	}
