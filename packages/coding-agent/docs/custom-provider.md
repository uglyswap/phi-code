...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. phi waits for the factory before startup continues, so the provider is available during interactive startup and to `phi --list-models`.
...
Credentials are persisted in `~/.phi/agent/auth.json`:
...
- [anthropic.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/anthropic.ts) - Anthropic Messages API
- [mistral.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/mistral.ts) - Mistral Conversations API
- [openai-completions.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/openai-completions.ts) - OpenAI Chat Completions
- [openai-responses.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/openai-responses.ts) - OpenAI Responses API
- [google.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/google.ts) - Google Generative AI
- [amazon-bedrock.ts](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/providers/amazon-bedrock.ts) - AWS Bedrock
...
- `stopReason === "error"`
- `errorMessage` matches one of pi's known overflow patterns (see [`packages/ai/src/utils/overflow.ts`](https://github.com/uglyswap/phi-code/blob/main/packages/ai/src/utils/overflow.ts))
...
Test your provider against the same test suites used by built-in providers. Copy and adapt these test files from [packages/ai/test/](https://github.com/uglyswap/phi-code/tree/main/packages/ai/test):
