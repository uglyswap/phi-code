...
| `phi-code-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `phi-code-tui` | TUI components for custom rendering |
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@phi-code-admin/phi-code";
...
```typescript
import type { AutocompleteItem } from "phi-code-tui";
...
```typescript
import { Box, Text } from "phi-code-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `phi-code-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "phi-code-ai";
...
**Important:** Use `StringEnum` from `phi-code-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
- [read.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/read.ts) - `ReadToolDetails`
- [bash.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/bash.ts) - `BashToolDetails`
- [edit.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/edit.ts)
- [write.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/write.ts)
- [grep.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/grep.ts) - `GrepToolDetails`
- [find.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/find.ts) - `FindToolDetails`
- [ls.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/core/tools/ls.ts) - `LsToolDetails`
...
Tools can provide `renderCall` and `renderResult` for custom TUI display. See [tui.md](tui.md) for the full component API and [tool-execution.ts](https://github.com/uglyswap/phi-code/blob/main/packages/coding-agent/src/modes/interactive/components/tool-execution.ts) for how tool rows are composed.
...
```typescript
import { Text, Component } from "phi-code-tui";
...
import { matchesKey } from "phi-code-tui";
