...
```typescript
import { createAgentSession, SessionManager } from "@phi-code-admin/phi-code";
...
The `Agent` class (from `phi-code-agent`) handles the core LLM interaction. Access it via `session.agent`.
...
import { ModelRuntime } from "@phi-code-admin/phi-code";

const modelRuntime = await ModelRuntime.create();

// create() restores cached catalogs but does not refresh them from the provider catalogs by default.
...
Remote catalogs are persisted locally so later runtimes can restore them without a network request. The default file is `~/.phi/agent/models-store.json`; set `modelsStorePath` to choose another location, or inject `modelsStore` to control persistence. Network refreshes are throttled to once per provider every four hours unless forced. To force an immediate refresh, call `await modelRuntime.refresh({ allowNetwork: true, force: true, signal })`. Setting `PI_OFFLINE` disables model network access.
...
```typescript
import { InMemoryCredentialStore } from "phi-code-ai";
import { createAgentSession, ModelRuntime } from "@phi-code-admin/phi-code";
...
// Or inject any phi-ai CredentialStore
...
The `edit` tool returns `details.diff` for phi's TUI display and `details.patch` as a standard unified patch for SDK consumers.
...
```typescript
import type { InlineExtension } from "@phi-code-admin/phi-code";
...
// Auth and Models
ModelRuntime // implements phi-ai Models and owns credential storage
