...
phi requests extended key reporting automatically when Kitty keyboard protocol is not available. With `extended-keys-format csi-u`, tmux forwards modified keys in CSI-u format, which is the most reliable configuration. The `extended-keys-format` option requires tmux 3.5 or later.
...
With tmux 3.2 through 3.4, omit `extended-keys-format csi-u`; phi still supports tmux's default xterm `modifyOtherKeys` format.
