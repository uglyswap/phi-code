...
```bash
npm install phi-code-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from 'phi-code-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from 'phi-code-telemetry';
...
`phi-code-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from 'phi-code-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from 'phi-code-agent';
...
- `phi-code-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `phi-code-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `phi-code-agent` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
