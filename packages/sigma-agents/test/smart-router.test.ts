...
describe("SmartRouter", () => {
	...
	test("classifyTask should handle case-insensitive matching", () => {
		const prompts = ["WRITE A FUNCTION", "Write A Function", "write a function", "WrItE a FuNcTiOn"];
	...
	test("getRecommendation should return default config for unknown category", () => {
		// Mock a router with incomplete configuration (cast to bypass exhaustive route check)
		const incompleteConfig = {
			routes: {} as RoutingConfig["routes"],
			default: {
	...
	test("French keywords should work correctly", () => {
		...
		const expectedCategories: TaskCategory[] = [
			"code",
			"code",
			"code", // développer, coder, programmer
			"debug",
			"debug", // réparer, corriger
			"explore",
			"explore", // analyser, comprendre
			"plan",
			"plan", // planifier, concevoir
			"test",
			"test", // tester, vérifier
			"review",
			"review", // réviser, améliorer
		];
