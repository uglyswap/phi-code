...
describe("NodeExecutionEnv", () => {
	...
	it.skipIf(!symlinkSupported)("lists symlinks as symlinks", async () => {
		const root = createTempDir();
		const env = new NodeExecutionEnv({ cwd: root });
	...
	// The assertion compares $PWD against a native path. On Windows the shell used by
	// exec() is an MSYS bash that reports a POSIX path (/tmp/...), so the comparison is
	// about shell flavour, not about exec() honouring cwd and env.
