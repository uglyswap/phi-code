...
describe("AgentHarness tools", () => {
	...
	describe("edit", () => {
		...
		it.skipIf(!symlinkSupported)("serializes concurrent edits through canonical and symlink paths", async () => {
		...
		it.skipIf(!symlinkSupported)("edits regular files through symlinks", async () => {
			const context = createContext();
	...
	describe("bash", () => {
		...
		// $PWD comes back as a POSIX path under the MSYS bash used on Windows, so the
		// native-path comparison below is about shell flavour, not about the tool
		// honouring cwd and env (covered by the other cases).
